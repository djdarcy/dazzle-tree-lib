#!/bin/bash
#
# Manual version update script for DazzleTreeLib
...
if [ "$AUTO_MODE" = false ]; then
    ...
    echo -e "${BLUE}DazzleTreeLib Version Updater${NC}"
    echo "=============================="
else
    RED=''
    GREEN=''
...
# Configuration
SOURCE_FILE="dazzletreelib/version.py"
...
if [ ! -f "$SOURCE_FILE" ]; then
    ...
    if [ -f "../$SOURCE_FILE" ]; then
        cd ..
    elif [ -f "../dazzletreelib/version.py" ]; then
        cd ..
        SOURCE_FILE="dazzletreelib/version.py"
    else
